...
class DensityBasedSampler(object):
    ...
    def __init__(self, n_samples, annFileName="none.ann", alpha=1, beta=1, random_state=1234, use_pca=False,
                 pca_dim=50):
    ...
    def fit_sample(self, data: np.ndarray, label=None, return_others=True, selection=None, entropy=None):
        if type(data) == list:
            data = np.array(data)
        ...
        if self.N <= self.n_samples:
            return [True] * self.N
        # np.random.seed(42)
        selection = self._fit_sample(data, label=label, selection=selection, entropy=entropy)
        if return_others:
            return selection, self.estimated_density, self.prob
        else:
            return selection

    def _fit_sample(self, data: np.ndarray, label=None, selection=None, entropy=None):
        ...
        self.prob = self.radius_of_k_neighbor + self.beta * entropy  # 采样概率与r和类标混杂度成正比
